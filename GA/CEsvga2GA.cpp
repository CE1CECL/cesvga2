...
#endif
static bool ceDecodePixelFormat(IOFourCharCode pixelFormat, ...);
static GAType* _allocGAType(CFUUIDRef factoryID);
static void _deallocGAType(GAType *myInstance);
static void _buildGAFTbl();
void* CEsvga2GAFactory(CFAllocatorRef allocator, CFUUIDRef typeID);
...
static HRESULT ceQueryInterface(void *myInstance, REFIID iid, LPVOID *ppv)
{
...
static ULONG ceAddRef(void *myInstance)
{
	GAType* me = static_cast<GAType*>(myInstance);
...
static ULONG ceRelease(void *myInstance)
{
	GAType* me = static_cast<GAType*>(myInstance);
...
static IOReturn ceProbe(void* myInstance, CFDictionaryRef propertyTable, io_service_t service, SInt32* order)
{
	if (order)
	   *order = 2000;
...
static IOReturn ceStart(void* myInstance, CFDictionaryRef propertyTable, io_service_t service)
{
	GAType* me = static_cast<GAType*>(myInstance);
	IOReturn rc;
...
static IOReturn ceStop(void* myInstance)
{
	GAType* me = static_cast<GAType*>(myInstance);
...
static IOReturn ceGetCapabilities(void* myInstance, FourCharCode select, CFTypeRef* capabilities)
{
	...
	if (select == IO_FOUR_CHAR_CODE('smvl')) {
		...
		return IOConnectCallMethod(me->_context, kIOCE2DSetMacrovision,
								   &input, 1,
								   0, 0,
								   0, 0,
								   0, 0);
...
static IOReturn ceAllocateSurface(void* myInstance, IOOptionBits options, IOBlitSurface* surface, void* cgsSurfaceID)
{
	...
	if (options & kIOBlitHasCGSSurface) {
		...
		rc = ceSetSurface(myInstance, 0x800U, surface);
		if (rc != kIOReturnSuccess)
		...
		if (flag) {
			...
			rc = IOConnectCallMethod(me->_context, kIOCE2DScaleSurface,
									 &input[0], 3,
									 0, 0,
									 0, 0,
									 0, 0);
	...
	if (!(options & kIOBlitReferenceSource)) {
		ceDecodePixelFormat(surface->pixelFormat /* eax */,
							&si->d2[5] /* ecx */,
							&si->d2[10] /* edx */,
							&tmp,	/* [ebp-0x18] */
							&si->d2[6],
...
static IOReturn ceFreeSurface(void* myInstance, IOOptionBits options, IOBlitSurface* surface)
{
	...
	if (me->_surface) {
		si->cgsSurfaceID = 0;
		rc = ceSetDestination(myInstance, 0, 0);
	}
	if (si->d0) {
		input = *(si->d0);
		rc = IOConnectCallMethod(me->_context, kIOCE2DDeleteImage,
								 &input, 1,
								 0, 0,
								 0, 0,
								 0, 0);
...
static IOReturn ceLockSurface(void* myInstance, IOOptionBits options, IOBlitSurface* surface, vm_address_t* address)
{
	...
	input = options;
	struct_out_cnt = sizeof struct_out;
	rc = IOConnectCallMethod(me->_context, kIOCE2DLockMemory,
							 &input, 1,
							 0, 0,
							 0, 0,
							 &struct_out[0], &struct_out_cnt);
...
static IOReturn ceUnlockSurface(void* myInstance, IOOptionBits options, IOBlitSurface* surface, IOOptionBits* swapFlags)
{
	...
	input = options;
	output_cnt = 1;
	rc = IOConnectCallMethod(me->_context, kIOCE2DUnlockMemory,
							 &input, 1,
							 0, 0,
							 &output, &output_cnt,
							 0, 0);
...
static IOReturn ceSwapSurface(void* myInstance, IOOptionBits options, IOBlitSurface* surface, IOOptionBits* swapFlags)
{
	...
	output_cnt = 1;
	input = options;
	rc = IOConnectCallMethod(me->_context, kIOCE2DSwapSurface,
							 &input, 1,
							 0, 0,
							 &output, &output_cnt,
							 0, 0);
...
static IOReturn ceSetDestination(void* myInstance, IOOptionBits options, IOBlitSurface* surface)
{
	...
	return ceSetSurface(myInstance, options, surface);
}
		...
		case kIOBlitTypeCopyRegion:
			switch (sourceType) {
				...
				case kIOBlitSourceCGSSurface:
					*blitter = &ceCopyRegion;
					rc = kIOReturnSuccess;
					break;
#ifdef SUPPORT_CGLS
				case kIOBlitSourceMemory:
				case kIOBlitSourceOOLMemory:
					*blitter = &ceMemCopyRegion;
					rc = kIOReturnSuccess;
					break;
#endif
			}
...
#endif
	input = options;
	return IOConnectCallMethod(me->_context, kIOCE2DFinish,
							   &input, 1,
							   0, 0,
							   0, 0,
							   0, 0);
...
static IOReturn ceWaitSurface(void* myInstance, IOOptionBits options, IOBlitSurface* surface)
{
	...
	if (si->d0 == 0)
		return kIOReturnSuccess;
	input = *(si->d0);
	return IOConnectCallMethod(me->_context, kIOCE2DWaitImage,
							   &input, 1,
							   0, 0,
							   0, 0,
							   0, 0);
...
static IOReturn ceSetSurface(void* myInstance, IOOptionBits options, IOBlitSurface* surface)
{
	...
	rc = IOConnectCallMethod(me->_context, kIOCE2DSetSurface,
							 &input[0], 2,
							 0, 0,
							 0, 0,
	...
	output_struct_cnt = sizeof output_struct;
	input_struct[0] = 288;
	rc = IOConnectCallMethod(me->_context, kIOCE2DReadConfigEx,
							 0, 0,
...
static void _buildGAFTbl()
{
	if (ga_initialized)
		return;
	bzero(&ga, sizeof ga);
	ga.QueryInterface = ceQueryInterface;
	ga.AddRef = ceAddRef;
	ga.Release = ceRelease;
	...
	ga.Stop = ceStop;
	ga.Reset = ceReset;
	ga.CopyCapabilities = ceGetCapabilities;
	ga.Flush = ceFlush;
	ga.Synchronize = ceSynchronize;
	ga.GetBeamPosition = ceGetBeamPosition;
	ga.AllocateSurface = ceAllocateSurface;
	ga.FreeSurface = ceFreeSurface;
	ga.LockSurface = ceLockSurface;
	ga.UnlockSurface = ceUnlockSurface;
	ga.SwapSurface = ceSwapSurface;
	ga.SetDestination = ceSetDestination;
	ga.GetBlitter = ceGetBlitter;
	ga.WaitComplete = ceWaitComplete;
	ga.__gaInterfaceReserved[0] = reinterpret_cast<void*>(ceWaitSurface);
	ga.__gaInterfaceReserved[1] = reinterpret_cast<void*>(ceSetSurface);
	/*
...
void* CEsvga2GAFactory(CFAllocatorRef allocator, CFUUIDRef typeID)
{
	if (CFEqual(typeID, kIOGraphicsAcceleratorTypeID))
		return _allocGAType(kCEGAFactoryID);
	return NULL;
}
