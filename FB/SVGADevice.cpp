...
bool CLASS::Start(IOPCIDevice* provider)
{
	...
	/*
	 * Note: in CECLGfx 5.x logLevel of following printouts is 0
	...
	m_cursor_ptr = 0;
	provider->setProperty("CECLSVGACapabilities", static_cast<uint64_t>(m_capabilities), 32U);
	return true;
}
...
bool CLASS::FIFOInit()
{
	...
	m_provider->setProperty("CECLSVGAFIFOCapabilities", static_cast<uint64_t>(fifo_capabilities), 32U);
...
{
	if (checkOptionFB(CE1_OPTION_FB_CURSOR_BYPASS_2)) {	// Added
		// CURSOR_BYPASS_2
...
bool CLASS::BeginVideoSetRegs(uint32_t streamId, size_t numItems, struct SVGAEscapeVideoSetRegs **setRegs)
{
	...
	cmd = static_cast<SVGAEscapeVideoSetRegs*>(FIFOReserveEscape(SVGA_ESCAPE_NSID_CECL, cmd_size));
	if (!cmd)
		return false;
	cmd->header.cmdType = SVGA_ESCAPE_CECL_VIDEO_SET_REGS;
...
bool CLASS::VideoFlush(uint32_t streamId)
{
	...
	cmd = static_cast<SVGAEscapeVideoFlush*>(FIFOReserveEscape(SVGA_ESCAPE_NSID_CECL, sizeof *cmd));
	if (!cmd)
		return false;
	cmd->cmdType = SVGA_ESCAPE_CECL_VIDEO_FLUSH;
...
void CLASS::RegDump()
{
	...
	for (uint32_t i = SVGA_REG_ID; i < SVGA_REG_TOP; ++i)
		regs[i] = ReadReg(i);
	m_provider->setProperty("CECLSVGADump", static_cast<void*>(&regs[0]), static_cast<unsigned>(sizeof regs));
}
