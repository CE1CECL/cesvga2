...
#define CLASS CEsvga2
#define super IOFramebuffer
OSDefineMetaClassAndStructors(CEsvga2, IOFramebuffer);
...
unsigned __attribute__((visibility("hidden"))) ce1_options_fb = 0U;
...
{
	if (!checkOptionFB(CE1_OPTION_FB_FIFO_INIT))
...
IOReturn CLASS::getAttribute(IOSelect attribute, uintptr_t* value)
{
	...
	if (attribute == kIOHardwareCursorAttribute) {
		if (value)
			*value = checkOptionFB(CE1_OPTION_FB_FIFO_INIT) ? 1U : 0U;
		r = kIOReturnSuccess;
	} else
...
IOReturn CLASS::setDisplayMode(IODisplayModeID displayMode, IOIndex depth)
{
	...
	if (m_custom_mode_switched) {
		if (customMode.width == dme->width && customMode.height == dme->height)
			...
			LogPrintf(3, "%s: Not setting mode in physical hardware\n", __FUNCTION__);
	...
	if (checkOptionFB(CE1_OPTION_FB_REG_DUMP))	// Added
		svga.RegDump();						// Added
	IOLockUnlock(m_iolock);
	m_display_mode = displayMode;
	m_depth_mode = 0;
...
IOReturn CLASS::CustomMode(CustomModeData const* inData, CustomModeData* outData, size_t inSize, size_t* outSize)
{
	...
	if (inData->flags & 1U) {
		...
		/*
		 * Following log eliminated in CECLGfx 5.x
		 */
...
bool CLASS::start(IOService* provider)
{
	...
	IOLog("IOFB: start\n");
	CELog_SendString("log IOFB: start\n");
	if (PE_parse_boot_argn("ce1_log_fb", &boot_arg, sizeof boot_arg))
	...
	if (PE_parse_boot_argn("ce1_fps", &boot_arg, sizeof boot_arg) &&
		boot_arg && boot_arg <= 100U)
		m_refresh_quantum_ms = 1000U / boot_arg;
	setProperty("CECLSVGARefreshQuantumMS", static_cast<uint64_t>(m_refresh_quantum_ms), 32U);
	 ...
	 * Begin Added
	 */
	if (checkOptionFB(CE1_OPTION_FB_REG_DUMP))
		svga.RegDump();
	...
	/*
	 * Note: CECLGfx 5.x also checks that
	...
	m_custom_switch = 0U;
	m_custom_mode_switched = false;
	if (checkOptionFB(CE1_OPTION_FB_FIFO_INIT)) {	// Added
		if (!svga.FIFOInit()) {
			LogPrintf(1, "%s: Failed FIFOInit.\n", __FUNCTION__);
			goto fail;
		}
		if (!svga.HasCapability(SVGA_CAP_TRACES) && checkOptionFB(CE1_OPTION_FB_REFRESH_TIMER))	// Added
...
/*
 * Note: this method of impersonating CECL's driver(s)
 *   doesn't work on OS 10.6, since Apple have made
 *   passiveMatch() private.  It's physical only in
 *   the 32-bit kernel.
 *
 * There's an alternative way to impersonate by creating
 *   a fake meta-class for classnames CECLIOFramebuffer/CECLGfx
...
bool CLASS::passiveMatch(OSDictionary* matching, bool changesOK)
{
	...
	if (!str)
		goto done;
	if (str->isEqualTo("CECLIOFramebuffer") || str->isEqualTo("CECLGfx")) {
		LogPrintf(2, "%s: matched against CECLIOFramebuffer/CECLGfx\n", __FUNCTION__);
...
{
	return checkOptionFB(CE1_OPTION_FB_FIFO_INIT) && checkOptionFB(CE1_OPTION_FB_ACCEL);
}
...
void CLASS::useAccelUpdates(bool state)
{
	...
	setProperty("CECLSVGAAccelSynchronize", state);
