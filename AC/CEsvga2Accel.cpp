...
void CLASS::timeSyncs()
{
	...
	if (current_stamp >= last_stamp + 60) {
		...
		if (m_framebuffer)
			m_framebuffer->setProperty("CECLSVGASyncFrequency", static_cast<uint64_t>(freq * 1000.0F), 64U);
	}
}
#endif
...
//	ce1_options_ac = CE1_OPTION_AC_GL_CONTEXT | CE1_OPTION_AC_2D_CONTEXT | CE1_OPTION_AC_SURFACE_CONNECT;
	...
	if (PE_parse_boot_argn("-svga3d", &boot_arg, sizeof boot_arg))
		ce1_options_ac |= CE1_OPTION_AC_SVGA3D;
	if (PE_parse_boot_argn("-ce1_no_yuv", &boot_arg, sizeof boot_arg))
		ce1_options_ac |= CE1_OPTION_AC_NO_YUV;
	if (PE_parse_boot_argn("-ce1_direct_blit", &boot_arg, sizeof boot_arg))
		ce1_options_ac |= CE1_OPTION_AC_DIRECT_BLIT;
	if (PE_parse_boot_argn("-ce1_no_screen_object", &boot_arg, sizeof boot_arg))
		ce1_options_ac |= CE1_OPTION_AC_NO_SCREEN_OBJECT;
	if (PE_parse_boot_argn("-ce1_no_gl", &boot_arg, sizeof boot_arg))
		ce1_options_ac &= ~CE1_OPTION_AC_GL_CONTEXT;
	if (PE_parse_boot_argn("-ce1_qe", &boot_arg, sizeof boot_arg))
		ce1_options_ac |= CE1_OPTION_AC_QE;
	if (checkOptionAC(CE1_OPTION_AC_QE))
		ce1_options_ac |= CE1_OPTION_AC_GL_CONTEXT;
	setProperty("CECLSVGAAccelOptions", static_cast<uint64_t>(ce1_options_ac), 32U);
	if (PE_parse_boot_argn("ce1_options_ga", &boot_arg, sizeof boot_arg)) {
		m_options_ga = boot_arg;
		setProperty("CECLSVGAGAOptions", static_cast<uint64_t>(m_options_ga), 32U);
	}
	if (PE_parse_boot_argn("ce1_log_ac", &boot_arg, sizeof boot_arg))
		m_log_level_ac = static_cast<int>(boot_arg);
	setProperty("CECLSVGAAccelLogLevel", static_cast<uint64_t>(m_log_level_ac), 32U);
	if (PE_parse_boot_argn("ce1_log_ga", &boot_arg, sizeof boot_arg)) {
		m_log_level_ga = static_cast<int>(boot_arg);
		setProperty("CECLSVGAGALogLevel", static_cast<uint64_t>(m_log_level_ga), 32U);
	}
	if (PE_parse_boot_argn("ce1_log_gld", &boot_arg, sizeof boot_arg)) {
		m_log_level_gld = static_cast<int>(boot_arg);
		setProperty("CECLSVGAGLDLogLevel", static_cast<uint64_t>(m_log_level_gld), 32U);
	}
}
...
bool CLASS::start(IOService* provider)
{
	...
	if (!super::start(provider))
		return false;
	IOLog("IOAC: start\n");
	CELog_SendString("log IOAC: start\n");
	processOptions();
	/*
	 * TBD: is there a possible race condition here where CEsvga2Accel::start
	 *   is called before CEsvga2 gets attached to its provider?
	 */
	...
	m_allocator = CEsvga2Allocator::factory();
	if (!m_allocator) {
	...
	if (!checkOptionAC(CE1_OPTION_AC_NO_SCREEN_OBJECT) && screen.Init(m_svga)) {
	...
	if ((bHaveScreenObject || checkOptionAC(CE1_OPTION_AC_SVGA3D | CE1_OPTION_AC_GL_CONTEXT)) &&
		svga3d.Init(m_svga)) {
	...
	if (checkOptionAC(CE1_OPTION_AC_NO_YUV))
		ACLog(1, "YUV Off\n");
	if (checkOptionAC(CE1_OPTION_AC_GL_CONTEXT) && !HaveGLBaseline()) {
		/*
		 * Disable GL User Clients
		 */
		ACLog(1, "Disabling OpenGL support due to lack of baseline capabilities\n");
		ce1_options_ac &= ~(CE1_OPTION_AC_GL_CONTEXT | CE1_OPTION_AC_QE);
		setProperty("CECLSVGAAccelOptions", static_cast<uint64_t>(ce1_options_ac), 32U);
	}
#if __ENVIRONMENT_MAC_OS_X_VERSION_MIN_REQUIRED__ >= 1060
	if (checkOptionAC(CE1_OPTION_AC_GL_CONTEXT)) {
	...
	if (checkOptionAC(CE1_OPTION_AC_GL_CONTEXT)) {
#ifdef USE_OWN_GLD
		setProperty("IOGLBundleName", "CEsvga2GLDriver");
#else
							  ...
							  IOUserClient ** handler)
{
	...
	switch (type) {
		case kIOAccelSurfaceClientType:
			if (!checkOptionAC(CE1_OPTION_AC_SURFACE_CONNECT))
			...
			client = CEsvga2Surface::withTask(owningTask, securityID, type);
			break;
		case 1:
			if (!checkOptionAC(CE1_OPTION_AC_GL_CONTEXT))
				return kIOReturnUnsupported;
			client = CEsvga2GLContext::withTask(owningTask, securityID, type);
			break;
		case 2:
			if (!checkOptionAC(CE1_OPTION_AC_2D_CONTEXT))
				return kIOReturnUnsupported;
			client = CEsvga22DContext::withTask(owningTask, securityID, type);
			break;
		case 3:
			if (!checkOptionAC(CE1_OPTION_AC_DVD_CONTEXT))
				return kIOReturnUnsupported;
			client = CEsvga2DVDContext::withTask(owningTask, securityID, type);
			break;
#if __ENVIRONMENT_MAC_OS_X_VERSION_MIN_REQUIRED__ >= 1060
		case 4:
			if (!checkOptionAC(CE1_OPTION_AC_GL_CONTEXT))
				return kIOReturnUnsupported;
			client = CEsvga2Device::withTask(owningTask, securityID, type);
			break;
		case 5:
			if (!checkOptionAC(CE1_OPTION_AC_GL_CONTEXT))
				return kIOReturnUnsupported;
			client = CEsvga2OCDContext::withTask(owningTask, securityID, type);
			break;
#endif
		default:
			return kIOReturnUnsupported;
						   ...
						   size_t regionSize)
{
	...
	m_framebuffer->lockDevice();
	rect = &rgn->bounds;
	if (checkOptionAC(CE1_OPTION_AC_REGION_BOUNDS_COPY)) {
		copyRect[0] = rect->x;
		copyRect[1] = rect->y;
...
HIDDEN
CEsvga2Surface* CLASS::findSurfaceForID(uint32_t surface_id)
{
	FindSurface fs;
	...
	return OSDynamicCast(CEsvga2Surface, fs.client);
}
