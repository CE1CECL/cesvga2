...
static inline
int select_default_format(CEsvga2TextureBuffer* tx, int usage)
{
...
static
bool need_yuv_shadow(CEsvga2Accel* provider, int surface_format)
{
...
{
	int i, count;
	CEsvga2TextureBuffer* del_txs[21];
	if (m_fbo[0])
		touchDrawFBO();
	for (i = 0; i != 2; ++i)
		if (m_fbo[i]) {
...
HIDDEN
void CLASS::addTextureToStream(CEsvga2TextureBuffer* tx)
{
	/*
	 * TBD
	 */
...
HIDDEN
void CLASS::dirtyTexture(CEsvga2TextureBuffer* tx, uint8_t face, uint8_t mipmap)
{
	if (!tx || !tx->sys_obj)
		return;
...
HIDDEN
void CLASS::get_tex_data(CEsvga2TextureBuffer* tx, uint32_t* tex_gart_address, uint32_t* tex_pitch, int usage)
{
	switch (tx->sys_obj_type) {
		case TEX_TYPE_SURFACE:
...
HIDDEN
IOReturn CLASS::create_host_surface_for_texture(CEsvga2TextureBuffer* tx)
{
	IOReturn rc;
...
HIDDEN
IOReturn CLASS::alloc_and_load_texture(CEsvga2TextureBuffer* tx)
{
	IOReturn rc;
		...
		case TEX_TYPE_OOB:
			for (hostImage.face = 0U;
				...
				for (gld_th = headers, hostImage.mipmap = 0U;
					...
					/*
					 * Note: This is a workaround for a bug in CECL backend.
...
HIDDEN
IOReturn CLASS::tex_subimage_2d(CEsvga2TextureBuffer* tx,
