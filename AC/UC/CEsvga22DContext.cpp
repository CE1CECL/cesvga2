...
#define CLASS CEsvga22DContext
#define super IOUserClient
OSDefineMetaClassAndStructors(CEsvga22DContext, IOUserClient);
...
static
IOExternalMethod const iofbFuncsCache[kIOCE2DNumMethods] =
{
...
{0, reinterpret_cast<IOMethod>(&CEsvga2Accel::UpdateFramebufferAutoRing), kIOUCScalarIStructI, 0, 4U * sizeof(UInt32)}
};
...
{
	if (!targetP || index >= kIOCE2DNumMethods)
		return 0;
	switch (index) {
		case kIOCE2DUpdateFramebuffer:
			if (m_provider)
				*targetP = m_provider;
			else
				return 0;
...
{
	uint32_t cecl_pixel_format, apple_pixel_format;
	IOReturn rc;
	...
	/*
	 * Note: CECL_FOURCC_YV12 is not supported (planar 4:2:0 format)
	 */
	if (options & 0x400U) {
		cecl_pixel_format = CECL_FOURCC_UYVY;
		apple_pixel_format = kIO2vuyPixelFormat;
	} else if (options & 0x200U) {
		cecl_pixel_format = CECL_FOURCC_YUY2;
	...
	if (rc != kIOReturnSuccess)
		return rc;
	return m_surface_client->context_set_surface(cecl_pixel_format, apple_pixel_format);
}
