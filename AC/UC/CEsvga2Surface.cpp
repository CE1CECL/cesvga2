...
#define CLASS CEsvga2Surface
#define super IOUserClient
OSDefineMetaClassAndStructors(CEsvga2Surface, IOUserClient);
...
IOReturn CLASS::message(UInt32 type, IOService* provider, void* argument)
{
	...
	fs = static_cast<CEsvga2Accel::FindSurface*>(argument);
...
void CLASS::Start3D()
{
	...
	if (bHaveScreenObject)
		return;
	if (checkOptionAC(CE1_OPTION_AC_DIRECT_BLIT))
		bDirectBlit = true;
	else {
...
void CLASS::clear_yuv_to_black(void* buffer, vm_size_t size)
{
	...
	switch (m_video.cecl_pixel_format) {
 ...
 *   CECL's black colorkey requirement.
 *
 ...
 *   CECL SVGA II doesn't support this.  So we take an all-or-nothing
...
bool CLASS::setVideoRegs()
{
	...
	m_video.unit.format = m_video.cecl_pixel_format;
...
IOReturn CLASS::surface_read_lock_options(eIOAccelSurfaceLockBits options, IOAccelSurfaceInformation* info, size_t* infoSize)
{
	...
	if (!bHaveID || !isSourceValid())
		return kIOReturnNotReady;
	bzero(info, *infoSize);
	if (OSTestAndSet(ceSurfaceLockRead, &bIsLocked))
	...
	no_backing:
		OSTestAndClear(ceSurfaceLockRead, &bIsLocked);
		return kIOReturnNoMemory;
	}
finishup:
...
IOReturn CLASS::surface_write_lock_options(eIOAccelSurfaceLockBits options, IOAccelSurfaceInformation* info, size_t* infoSize)
{
	...
	if (OSTestAndSet(ceSurfaceLockWrite, &bIsLocked))
		return kIOReturnCannotLock;
skip:
	if (isClientBackingValid())
		goto finishup;
	if (!allocBacking() || !mapBacking(m_owning_task, 0U)) {
		OSTestAndClear(ceSurfaceLockWrite, &bIsLocked);
		return kIOReturnNoMemory;
	}
finishup:
	calculateSurfaceInformation(info);
...
IOReturn CLASS::surface_query_lock()
{
	...
	return (bIsLocked & ((128U >> ceSurfaceLockRead) | (128U >> ceSurfaceLockWrite))) != 0 ? kIOReturnCannotLock : kIOReturnSuccess;
}
											 ...
											 size_t rgnSize)
{
	...
	if (options & kIOAccelSurfaceShapeIdentityScaleBit) {
		if (m_wID != 1U ||
			checkOptionAC(CE1_OPTION_AC_PACKED_BACKING)) {
			m_scale.source.w = rgn->bounds.w;
			m_scale.source.h = rgn->bounds.h;
			m_scale.buffer.x = 0;
			m_scale.buffer.y = 0;
...
#pragma mark -
#pragma mark CEsvga22DContext Support Methods
#pragma mark -

HIDDEN
IOReturn CLASS::context_set_surface(uint32_t cecl_pixel_format, uint32_t apple_pixel_format)
{
	SFLog(2, "%s[%#x](%#x, %#x)\n", __FUNCTION__, m_wID, cecl_pixel_format, apple_pixel_format);

	if (!cecl_pixel_format) {
		surface_video_off();
		bVideoMode = false;
	...
	m_video.cecl_pixel_format = cecl_pixel_format;
	switch (cecl_pixel_format) {
		case CECL_FOURCC_UYVY:
			m_surfaceFormat = SVGA3D_UYVY;
			break;
		case CECL_FOURCC_YUY2:
...
IOReturn CLASS::context_lock_memory(task_t context_owning_task, mach_vm_address_t* address, mach_vm_size_t* rowBytes)
{
	...
	if (!bHaveID || !isSourceValid())
		return kIOReturnNotReady;
	if (OSTestAndSet(ceSurfaceLockContext, &bIsLocked))
		return kIOReturnCannotLock;
	if (!allocBacking() ||
		!mapBacking(context_owning_task, 1U)) {
		OSTestAndClear(ceSurfaceLockContext, &bIsLocked);
		return kIOReturnNoMemory;
	}
	if (bVideoMode &&
...
HIDDEN
IOReturn CLASS::copy_surface_region_to_self(class CEsvga2Surface* source_surface,
											int destX,
											int destY,
											IOAccelDeviceRegion const* region,
											size_t regionSize)
...
#pragma mark -
#pragma mark CEsvga2GLContext Support Methods
#pragma mark -
