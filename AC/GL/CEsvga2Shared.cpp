...
#define CLASS CEsvga2Shared
#define super OSObject
OSDefineMetaClassAndStructors(CEsvga2Shared, OSObject);
...
HIDDEN
IOReturn mapGLDTextureHeader(CEsvga2TextureBuffer* tx, IOMemoryMap** map)
{
	IOMemoryMap* mmap;
...
HIDDEN
void CLASS::delete_texture_internal(class CEsvga2Accel* provider,
									CEsvga2Shared* shared,
									CEsvga2TextureBuffer* texture)
{
...
HIDDEN
void CLASS::free_orphan_texture(class CEsvga2Accel*, struct IOTextureBuffer*)
{
	/*
	 * TBD
	 */
...
HIDDEN
void CLASS::finalize_texture(class CEsvga2Accel* provider, CEsvga2TextureBuffer* texture)
{
...
int isUserMemoryReadOnly(task_t owningTask, mach_vm_address_t address, mach_vm_size_t size)
{
	...
	_address = address;
	_size = size;
	info_count = CE_REGION_BASIC_INFO_COUNT_64;
	if (mach_vm_region(get_task_map(owningTask),
					   &_address,
					   &_size,
					   CE_REGION_BASIC_INFO_64,
					   reinterpret_cast<vm_region_info_t>(&info),
					   &info_count,
					   0) != ERR_SUCCESS)
		return -1;
	return !(info.protection & CE_PROT_WRITE);
}

HIDDEN
CEsvga2TextureBuffer* CLASS::new_agp_texture(mach_vm_address_t pixels,
											 size_t texture_size,
											 uint32_t read_only,
...
HIDDEN
void CLASS::link_texture_at_head(CEsvga2TextureBuffer* texture)
{
	if (m_texture_list)
 ...
 */
HIDDEN
void CLASS::unlink_nonhead_texture(CEsvga2TextureBuffer* texture)
{
...
HIDDEN
void CLASS::unlink_texture(CEsvga2TextureBuffer* texture)
{
...
HIDDEN
CEsvga2TextureBuffer* CLASS::common_texture_init(uint8_t object_type)
{
	mach_vm_address_t client_addr;	// var_30
	CEsvga2TextureBuffer* p;
...
HIDDEN
CLASS* CLASS::factory(task_t owningTask, class CEsvga2Accel* provider, int logLevel)
{
	CLASS* obj = new CLASS;
...
bool CLASS::init(task_t owningTask, class CEsvga2Accel* provider, int logLevel)
{
	if (!super::init())
		return false;
...
HIDDEN
CEsvga2TextureBuffer* CLASS::new_surface_texture(uint32_t surface_id,
												 uint32_t fb_idx_mask,
												 mach_vm_address_t* sys_obj_client_addr)
{
	class CEsvga2Surface* surface;
	CEsvga2TextureBuffer* p;
#ifdef ATTACH_SURFACE_TEXTURE
	IOReturn rc;
#endif
...
HIDDEN
CEsvga2TextureBuffer* CLASS::new_iosurface_texture(uint32_t, uint32_t, uint32_t, uint32_t, mach_vm_address_t*)
{
	/*
	 * TBD
	 */
...
HIDDEN
CEsvga2TextureBuffer* CLASS::new_texture(uint32_t size0,
										 uint32_t size1,
										 ...
										 mach_vm_address_t* sys_obj_client_addr)
{
	CEsvga2TextureBuffer *p1 /* var_3c */, *p2;
	IOBufferMemoryDescriptor* bmd;
...
HIDDEN
CEsvga2TextureBuffer* CLASS::new_agpref_texture(mach_vm_address_t pixels1,
...
HIDDEN
IOReturn CLASS::pageoffDirtyTexture(CEsvga2TextureBuffer* tx)
{
	IOReturn rc;
...
HIDDEN
bool CLASS::initializeTexture(CEsvga2TextureBuffer* tx, VendorNewTextureDataStruc const* tds)
{
...
HIDDEN
CEsvga2TextureBuffer* CLASS::findTextureBuffer(uint32_t object_id)
{
	if (!m_handle_table || object_id >= m_num_handles)
		return 0;
	return static_cast<CEsvga2TextureBuffer*>(m_handle_table[object_id]);
}
